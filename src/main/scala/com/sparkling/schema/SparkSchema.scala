...
/** Converts sparkling schema types ([[com.sparkling.row.schema.ValueType ValueType]],
  * [[com.sparkling.row.schema.RecordSchema RecordSchema]]) to their Spark SQL equivalents (`DataType`, `StructType`).
  *
...
object SparkSchema {
  ...
  /** Convert a [[com.sparkling.row.schema.ValueType ValueType]] into a Spark SQL `DataType`. */
  ...
  /** Convert a [[com.sparkling.row.schema.RecordSchema RecordSchema]] into a Spark `StructType`. */
  def toStructType(schema: RecordSchema): SparkType.StructType =
    SparkType.StructType(schema.columns.map(toStructField))

  /** Convert a [[com.sparkling.row.schema.SchemaField SchemaField]] into a Spark `StructField`. */
